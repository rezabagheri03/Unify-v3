...
class GoldenScheduleController extends Controller
{
    public function generate(Request $request)
    {
        ...
        // Post-audit: one generator per (semester, hash) at a time — the shared
        // cache's whole point is defeated if a burst of identical misses each
        // pays the 5s combinatorial generation.
        $generate = function () use ($available, $preferences, $semester, $hash, $user) {
        ...
        try {
            // Up to 10s lock; wait at most ~5s for an in-flight twin to finish,
            // then re-check the cache it just filled.
            $suggestions = \Illuminate\Support\Facades\Cache::lock("golden:{$semester->id}:{$hash}", 10)
                ->block(5, fn () => GoldenScheduleCache::where('semester_id', $semester->id)
                        ->where('preferences_hash', $hash)
                        ->where('expires_at', '>', now())
                        ->first()?->combos ?? $generate());
        } catch (\Illuminate\Contracts\Cache\LockTimeoutException $e) {
            // Overloaded: degrade gracefully — generate without storing.
            $suggestions = (new GoldenSchedulerService())->generate($available->pluck('id')->all(), $preferences);
        }
