...
4. Upload Laravel backend: Zip `/unify-backend` outside `public_html`, unzip, run `composer install --no-dev`, set `.env` DB credentials, APP_KEY, PUSHE_API_KEY, KAVENEGAR_KEY, set `storage` link `php artisan storage:link` creates symlink `public_html/uploads -> storage/app/public`
