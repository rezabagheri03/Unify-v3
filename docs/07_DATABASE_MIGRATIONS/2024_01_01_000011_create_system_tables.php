...
return new class extends Migration {
    public function up() {
        ...
        Schema::create('audit_logs', function (Blueprint $table) {
            ...
            $table->enum('action', ['deletion','major_edit','password_reset','role_change','ban','honor_status_change','final_semester_abuse_flag','login','failed_login','file_upload','file_approval','message_edit_delete','ticket_status_change']);
            $table->string('resource_type', 50);
            $table->string('resource_id', 100);
            $table->dateTime('timestamp');
            $table->string('ip_address', 45)->nullable();
            $table->text('user_agent')->nullable();
            $table->longText('details')->nullable(); // encrypted via Crypt::encryptString
        ...
        Schema::create('notifications', function (Blueprint $table) {
            ...
            $table->string('user_id', 32);
            $table->foreign('user_id')->references('id')->on('users')->cascadeOnDelete();
            $table->string('type', 50); // spec_changed, resource_new, ticket_answered, etc.
        ...
        Schema::create('honor_flags', function (Blueprint $table) {
            ...
            $table->string('flag_type', 50); // final_semester_abuse, frequent_switch
