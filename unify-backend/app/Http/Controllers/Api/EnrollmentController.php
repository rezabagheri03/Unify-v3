    ...
    public function storeTemp(Request $request)
    {
        ...
        if ($idempotencyKey) {
            $existing = IdempotencyKeys::where('key', $idempotencyKey)
                ...
                ->first();
            // Round-2 (V2-17e): replay only UNEXPIRED keys — an expired key
            // (nightly purge hasn't run yet) replayed a stale "created" body
            // for an enrollment that may no longer exist.
