...
class BroadcastThrottleController extends Controller
{
    public function check(Request $request)
    {
        ...
        if ($throttle && $throttle->last_sent_at->diffInMinutes(now()) < 10) {
            ...
            // Round-2 (V2-13): exact remainder — a 60s floor overstated the
            // wait by up to 59s at the tail of the window.
            $remaining = max(1, 600 - (int) $throttle->last_sent_at->diffInSeconds(now()));
