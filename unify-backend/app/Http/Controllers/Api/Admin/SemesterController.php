...
class SemesterController extends Controller
{
    ...
    public function createNewSemester(Request $request)
    {
        ...
        // Round-2 (V2-10): fail with 422s, not 500s, on admin input errors.
        if (! ShamsiService::isValid($request->start_shamsi) || ! ShamsiService::isValid($request->end_shamsi)) {
            return response()->json(['message' => 'تاریخ شمسی نامعتبر است (قالب: 1403/06/25)', 'code' => 'BAD_DATE'], 422);
        }
        if (Semester::where('id', $request->name)->exists()) {
            return response()->json(['message' => 'نیم‌سالی با این نام از قبل وجود دارد', 'code' => 'DUPLICATE_SEMESTER'], 422);
        }
        $startG = ShamsiService::toGregorian($request->start_shamsi);
        $endG = ShamsiService::toGregorian($request->end_shamsi);
        if (strtotime((string) $endG) <= strtotime((string) $startG)) {
            return response()->json(['message' => 'تاریخ پایان باید بعد از شروع باشد', 'code' => 'BAD_RANGE'], 422);
        }
        ...
        DB::transaction(function () use ($request, $old, $startG, $endG) {
            if ($old) {
        ...
        // Round-2 (V2-10): the payload key is `name` — $request->id never
        // existed, so every audit row landed with resource_id '-'.
        \App\Models\AuditLog::record($request->user()->id, 'semester_created', 'semester', $request->name, $request);
