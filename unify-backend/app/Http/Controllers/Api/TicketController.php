...
class TicketController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($user->role === 'student') {
            ...
            // Round-2 (V2-02): lanes are REAL. tickets.department is the
            // FUNCTIONAL lane (education/technical/student_affairs) while
            // users.department_id is the ACADEMIC department — the old
            // comparison could never match, leaving expert inboxes silently
            // empty. Staff ownership of a lane lives on users.ticket_lane.
            if ($user->ticket_lane) {
                $query->where('department', $user->ticket_lane);
            } else {
                $query->whereRaw('1 = 0'); // no lane assigned → explicitly empty inbox
            }
        }
        // admin/owner: all
    ...
    public function store(Request $request)
    {
        ...
        // 5/day limit (F08). Round-2 (V2-11): the ticket cap was the last
        // check-then-act quota — two parallel stores at 4/5 both passed.
        // Now claims through the shared atomic DailyQuota primitive AFTER
        // validation, same as upload/download/version.
        if (! \App\Services\DailyQuota::claim(TicketDailyCount::class, 'student_id', $user->id, 5)) {
        ...
        // Round-2 (V2-03): SEC-04 everywhere — embedded users are trimmed to
        // public-safe columns; mobile/email never ride a ticket payload.
        return response()->json($ticket->load('student:' . \App\Models\User::PUBLIC_COLS), 201);
    }
    ...
    public function show(Request $request, $id)
    {
        $user = $request->user();
        $ticket = Ticket::with([
            'student:' . \App\Models\User::PUBLIC_COLS,
            'assignedExpert:' . \App\Models\User::PUBLIC_COLS,
            'replies.sender:' . \App\Models\User::PUBLIC_COLS,
        ])->findOrFail($id);
    ...
    public function reply(Request $request, $ticketId)
    {
        ...
        return response()->json($ticket->fresh(['replies.sender:' . \App\Models\User::PUBLIC_COLS]), 201);
    }
    ...
    public function updateStatus(Request $request, $ticketId)
    {
        ...
        if (! in_array($user->role, ['expert', 'admin', 'owner']) || ! $this->canHandle($user, $ticket)) {
            return response()->json(['message' => 'دسترسی ندارید', 'code' => 'FORBIDDEN'], 403);
        }
    ...
    private function canView($user, Ticket $ticket): bool
    {
        ...
        return $this->canHandle($user, $ticket);
    }

    /**
     * Round-2 (V2-02): staff handling is lane-scoped. admin/owner work every
     * lane; expert/head_of_dept only their assigned functional lane. A staff
     * account without a lane handles nothing (explicit, not silent-global).
     */
    private function canHandle($user, Ticket $ticket): bool
    {
        if (in_array($user->role, ['admin', 'owner'])) {
            return true;
        }
        if (! in_array($user->role, ['expert', 'head_of_dept'])) {
            return false;
        }
        return $user->ticket_lane !== null && $user->ticket_lane === $ticket->department;
    }
}
