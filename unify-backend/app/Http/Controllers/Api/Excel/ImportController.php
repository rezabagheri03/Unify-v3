...
class ImportController extends Controller
{
    ...
    public function importUsers(Request $request)
    {
        ...
        $headerMap = [
            ...
            // Round-2 (V2-02 provisioning): optional 7th column assigns the
            // functional ticket lane for staff rows (خ/ education etc.).
            'زمینه تیکت' => 'ticket_lane',
        ...
        foreach (array_slice($rows, 1) as $i => $row) {
            $rowNo = $i + 2;
            $row = array_pad($row, 7, null);
            [$id, $first, $last, $roleFa, $deptId, $statusFa, $laneEn] = $row;
                ...
                $lane = trim((string) $laneEn) ?: null;
                if ($lane && ! in_array($lane, ['education', 'technical', 'student_affairs'], true)) {
                    $errors[] = ['row' => $rowNo, 'error' => "زمینه تیکت نامعتبر: {$lane} (education/technical/student_affairs)", 'data' => $row];
                    continue;
                }

            $seenIds[$id] = true;
            $validated[] = compact('id', 'first', 'last', 'role', 'deptId', 'statusFa', 'lane');
        }
        ...
        try {
            ...
            DB::commit();
            \App\Models\AuditLog::record($request->user()?->id, 'users_imported', 'user', null, $request, ['created' => $created]);
            return response()->json(['message' => "ایمپورت موفق: {$created} کاربر اضافه شد", 'created' => $created]);
        } catch (\Throwable $e) {
            DB::rollBack();
            // Post-audit F-11: never leak raw exception text (SQL detail) to clients.
            // CI observability: class + file:line -> laravel.log -> leg failure dump.
            \Illuminate\Support\Facades\Log::error('user import failed', [
                'error' => $e->getMessage(),
                'exception' => $e::class,
                'at' => $e->getFile() . ':' . $e->getLine(),
            ]);
            return response()->json(['message' => 'ایمپورت ناموفق بود — گزارش سرور را بررسی کنید', 'code' => 'IMPORT_ERROR'], 500);
        }
    }
    ...
    public function importCourses(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            \App\Models\AuditLog::record($request->user()?->id, 'courses_imported', 'course', null, $request, ['created' => $created]);
    ...
    public function importSpecifications(Request $request)
    {
        ...
        $errors = [];
        $created = 0;
        // Round-2 (V2-09): preload valid semesters once — orphan specs die as
        // row errors instead of FK exceptions/500s.
        $semesterIds = \App\Models\Semester::pluck('id')->flip();
        DB::beginTransaction();
        try {
            foreach (array_slice($rows, 1) as $i => $row) {
                $rowNo = $i + 2;
                ...
                // Round-2: course_specifications.course_id FK-references
                // courses.ID — resolve the row and bind by id (the legacy
                // import wrote id==code, but nothing enforces that).
                $course = Course::where('code', trim((string) $courseCode))->first();
                if (! $course) {
                ...
                // Round-2 (V2-09): the overlap engine compares times
                // LEXICOGRAPHICALLY — '8:00' (unpadded) or garbage would
                // silently corrupt it on sqlite and rely on MySQL column
                // coercion on prod. Validate + normalize here, per row.
                $startN = $this->normalizeTime($start);
                $endN = $this->normalizeTime($end);
                if (! $startN || ! $endN) {
                    $errors[] = ['row' => $rowNo, 'error' => 'فرمت زمان نامعتبر (الگو: 08:00)', 'data' => $row];
                    continue;
                }
                if ($startN >= $endN) {
                    $errors[] = ['row' => $rowNo, 'error' => 'بازه زمانی نامعتبر — شروع باید قبل از پایان باشد', 'data' => $row];
                    continue;
                }
                $semId = trim((string) $semesterId) ?: '1403-1';
                if (! $semesterIds->has($semId)) {
                    $errors[] = ['row' => $rowNo, 'error' => "نیم‌سال نامعتبر: {$semId}", 'data' => $row];
                    continue;
                }

                CourseSpecification::create([
                    // VARCHAR(32) PK: dashless UUID, same as the factory —
                    // sqlite ignores the length but prod MySQL strict mode 500s.
            ...
            DB::commit();
            \App\Models\AuditLog::record($request->user()?->id, 'specifications_imported', 'course_specification', null, $request, ['created' => $created]);
    ...
    /** Round-2 (V2-09): strict H:MM(SS) → padded HH:MM, else null. */
