...
class AssignmentController extends Controller
{
    ...
    public function grade(Request $request, $id)
    {
        ...
        // Round-2 (V2-17d): a professor grades only trackers attached to
        // THEIR OWN class spec — the endpoint used to accept any tracker id
        // (trackers are student self-management data). expert/admin keep the
        // cross-class capacity.
        if ($user->role === 'professor') {
            $ownsSpec = \App\Models\CourseSpecification::where('id', $assignment->specification_id)
                ->where('professor_id', $user->id)
                ->exists();
            if (! $ownsSpec) {
                return response()->json(['message' => 'فقط تکالیف کلاس خودتان را نمره می‌دهید', 'code' => 'FORBIDDEN'], 403);
            }
        }
