...
class OfflineSyncController extends Controller
{
    ...
    public function sync(Request $request)
    {
        ...
        foreach ($data['items'] as $item) {
            $existing = IdempotencyKeys::where('key', $item['idempotency_key'])
                ...
                ->first();
            // Round-2 (V2-17e): an expired key must reprocess, not report a
            // stale duplicate — free it and fall through to normal handling.
    ...
    private function processRating(array $payload, string $userId)
    {
        ...
        // firstOrNew + explicit uuid ONLY on create: the PK is NOT NULL with no
        // default, and re-writing `id` on an update would churn the primary key.
        $rating = ResourceRating::firstOrNew([
            'student_id' => $userId,
            'resource_family_id' => $payload['resource_family_id'],
        ]);
        if (! $rating->exists) {
        ...
        $rating->fill([
            ...
            'rated_at' => now(),
            'is_self_rating' => $familyProfessor === $userId,
        ])->save();

        // Round-2 (V2-14): offline ratings recompute the family score too —
        // previously only the online endpoint did, so synced ratings left
        // averages stale indefinitely.
        \App\Services\ResourceRatingRecalc::run($payload['resource_family_id']);
    }
