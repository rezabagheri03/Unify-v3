...
class ResourceController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        // byte hits the disk.
        if (! $this->claimDailyQuota(ResourceUploadCount::class, 'user_id', $user->id, 5)) {
    ...
    public function download(Request $request, $id)
    {
        ...
        if (! $this->claimDailyQuota(DownloadDailyCount::class, 'student_id', $user->id, 20)) {
    ...
    /**
     * Post-audit F-16: atomically claim one unit of a daily quota.
     * The unique (user, date) row is locked while read+incremented, so two
     * parallel requests can no longer both pass at count == limit-1.
     */
    private function claimDailyQuota(string $modelClass, string $userColumn, string $userId, int $max): bool
    {
        $today = now()->toDateString();
        try {
        ...
        return \Illuminate\Support\Facades\DB::transaction(function () use ($modelClass, $row, $max) {
