...
import api, { apiErrorMessage, uuidv4 } from '../../api/client';
...
export default function SchedulerA() {
  ...
  // Post-audit F-07: one idempotency key per user-intent. Double-taps and
  // automatic retries of the same "add this spec" intent now carry the SAME
  // key, so the server replays the first response instead of 409-ing.
  const addIntentKeyRef = useRef<Record<string, string>>({});
  const intentKeyFor = (specId: string): string => {
    return (addIntentKeyRef.current[specId] ??= uuidv4());
  };
  ...
  const addTemp = async (specId: string) => {
    try {
      const res = await api.post(
        ...
        { headers: { 'Idempotency-Key': intentKeyFor(specId) } }
      );
      ...
      delete addIntentKeyRef.current[specId]; // intent settled — next add is new
      load();
    } catch (err: any) {
      delete addIntentKeyRef.current[specId]; // defensive: server 409/4xx also settles
