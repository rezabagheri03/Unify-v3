...
`database/database.sqlite` is already in the repo with demo accounts — kept
deliberately; it ships with seed data ONLY: token/audit runtime tables are
emptied before each release), so no database server or config is needed. Never
commit your local `*.sqlite-journal` / `*.sqlite-wal` runtime files.
